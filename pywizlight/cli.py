...
from pywizlight import PilotBuilder, discovery, tui, wizlight
...
@click.option(
    ...
    show_default=True,
    help="Broadcast address used for discovery (e.g. 192.168.1.255).",
)
@click.option(
    "--wait",
    "-w",
    ...
    type=float,
    show_default=True,
    help="Seconds to wait for discovery responses.",
)
def tui_command(broadcast: str, wait: float) -> None:
    """Launch a TUI to discover bulbs and manage power, scenes, brightness, and RGB."""
    try:
        tui.run_tui(broadcast_address=broadcast, wait_time=wait)
